...
Routing: /orders 요청은 주문 서버로, /users는 회원 서버로 교통 정리.
...
앱은 Gateway에 GET /main-page 한 번만 요청.

Gateway가 내부적으로 회원, 상품, 공지 서버를 찔러 데이터를 모음(Aggregation).

모은 데이터를 하나의 JSON으로 합쳐서 앱에 응답 (비빔밥 비유).
...
스펙 확인: 에러 발생 시 Swagger(API 문서)와 내가 보낸 데이터 비교.

도구 활용: Charles, ProxyMan, DevTools로 실제 오고 가는 JSON 확인.

구조적 의심: "데이터가 일부만 안 나오나?" → MSA 구조상 특정 서버(예: 추천 서버)만 지연되는지 확인.
